...
private fun String.fullyContained() = split(",").map { it.toNumberList() }
    .let { (first, second) -> first.containsAll(second) || second.containsAll(first) }

private fun String.overlap() = split(",").map { it.toNumberList().toSet() }.let { (numbers1, numbers2) -> numbers1.intersect(numbers2).isNotEmpty() }

private fun String.toNumberList() = split("-").let { (start, end) -> start.toLong()..end.toLong() }.toList()
